...
static void editor_grow(Editor* editor, size_t n) {
	...
	while (new_capacity - editor->size < n) {
		...
		memmove(editor->lines + (editor->cursor_row + 1) * sizeof(editor->lines[0]), editor->lines + (editor->cursor_row) * sizeof(editor->lines[0]), editor->size - (editor->cursor_row) * sizeof(editor->lines[0]));
	}
